// frontend/src/components/timetable/ExamBlock.tsx
import { useDrag } from 'react-dnd';
import type { RenderableExam } from '../../store/types';
...
export function ExamBlock({ exam, color, startColumn, spanColumns, stackLevel }: ExamBlockProps) {
  ...
  const getUtilization = () => {
    if (exam.roomCapacity === 0) return 'N/A';
    const percentage = Math.round((exam.expectedStudents / exam.roomCapacity) * 100);
    return `${percentage}%`;
  };

  const getUtilizationColor = () => {
    if (exam.roomCapacity === 0) return 'text-gray-400';
