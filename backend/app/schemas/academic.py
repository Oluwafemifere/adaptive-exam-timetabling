...
class AcademicSession(Base):
    ...
    semester_system = Column(
        String, nullable=False
    )  # "Two Semester", "Three Semester", etc.
    ...
    # Relationships
    exams = relationship(
        "Exam", back_populates="academic_session", cascade="all, delete-orphan"
    )
    ...
    timetable_jobs = relationship(
        "TimetableJob", back_populates="academic_session", cascade="all, delete-orphan"
    )
    staff_unavailability = relationship(
        "StaffUnavailability",
        back_populates="academic_session",
        cascade="all, delete-orphan",
    )
...
class Programme(Base):
    ...
    code = Column(
        String, nullable=False
    )  # Not unique as different departments might have similar codes
    ...
    students = relationship(
        "Student", back_populates="programme", cascade="all, delete-orphan"
    )

    # Unique constraint on code within department
    __table_args__ = (
        UniqueConstraint(
            "code", "department_id", name="unique_programme_code_per_department"
        ),
...
class Course(Base):
    ...
    )  # If true, must be scheduled in morning
    is_active = Column(Boolean, default=True, nullable=False)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
    updated_at = Column(
        DateTime(timezone=True), server_default=func.now(), onupdate=func.now()
    ...
    registrations = relationship(
        "CourseRegistration", back_populates="course", cascade="all, delete-orphan"
    )
...
class Student(Base):
    ...
    )  # regular, part-time, sandwich, etc.
    special_needs: Mapped[Optional[List[str]]] = mapped_column(
        ARRAY(String)
    )  # Array of special accommodations
...
class Staff(Base):
    ...
    max_daily_sessions = Column(
        Integer, default=2
    )  # Maximum invigilation sessions per day
    max_consecutive_sessions = Column(
        Integer, default=2
    ...
    invigilator_assignments = relationship(
        "ExamInvigilator", back_populates="staff", cascade="all, delete-orphan"
    )
    unavailability = relationship(
        "StaffUnavailability", back_populates="staff", cascade="all, delete-orphan"
    )
...
class CourseRegistration(Base):
    ...
    session_id = Column(
        UUID(as_uuid=True), ForeignKey("academic_sessions.id"), nullable=False
    )
...
class StaffUnavailability(Base):
    ...
    time_slot_id = Column(
        UUID(as_uuid=True), ForeignKey("time_slots.id")
    )  # Specific time slot or all day
