...
class FileUploadSession(Base, TimestampMixin):
    ...
    session_id: Mapped[uuid.UUID | None] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("academic_sessions.id"), nullable=True)
...
class UploadedFile(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    upload_session_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("file_upload_sessions.id"), nullable=False)
    ...
    validation_status: Mapped[str] = mapped_column(String, default="pending", nullable=False)
    validation_errors: Mapped[dict | None] = mapped_column(JSON, nullable=True)
