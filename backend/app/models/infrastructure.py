...
from sqlalchemy import String, Boolean, Integer, ForeignKey, JSON, ARRAY
...
class Room(Base):
    ...
    room_type_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("room_types.id"), nullable=False)
    ...
    accessibility_features: Mapped[list[str] | None] = mapped_column(ARRAY(String), nullable=True)
    is_active: Mapped[bool]    = mapped_column(Boolean, default=True, nullable=False)
...
class ExamRoom(Base):
    ...
    seating_arrangement: Mapped[dict | None] = mapped_column(JSON, nullable=True)
