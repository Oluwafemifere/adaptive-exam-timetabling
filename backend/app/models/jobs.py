...
from sqlalchemy import String, ForeignKey, Integer, Numeric, JSON, DateTime, Text, Boolean
...
class TimetableJob(Base, TimestampMixin):
    ...
    configuration_id: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("system_configurations.id"), nullable=False)
    initiated_by: Mapped[uuid.UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("users.id"), nullable=False)
    ...
    hard_constraint_violations: Mapped[int]    = mapped_column(Integer, default=0, nullable=False)
    soft_constraint_score: Mapped[Numeric | None] = mapped_column(Numeric, nullable=True)
    room_utilization_percentage: Mapped[Numeric | None] = mapped_column(Numeric, nullable=True)
    ...
    version: Mapped["TimetableVersion"]        = relationship(back_populates="job", uselist=False)

class TimetableVersion(Base, TimestampMixin):
    __tablename__ = "timetable_versions"

    id: Mapped[uuid.UUID]        = mapped_column(PG_UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    job_id: Mapped[uuid.UUID]    = mapped_column(PG_UUID(as_uuid=True), ForeignKey("timetable_jobs.id"), nullable=False, unique=True)
    version_number: Mapped[int]  = mapped_column(Integer, unique=True, nullable=False, default=1)
