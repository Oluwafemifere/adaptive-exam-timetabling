...
class ConstraintRule(Base, TimestampMixin):
    ...
    constraint_definition: Mapped[dict] = mapped_column(JSONB, nullable=False)
    category_id: Mapped[uuid.UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("constraint_categories.id"), nullable=False
    )
    default_weight: Mapped[Numeric] = mapped_column(
        Numeric, default=1.0, nullable=False
    )
    ...
    configurations: Mapped[list["ConfigurationConstraint"]] = relationship(
        back_populates="rule"
    )
...
class ConfigurationConstraint(Base):
    ...
    custom_parameters: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
