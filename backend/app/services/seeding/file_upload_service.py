# backend/app/services/uploads/file_upload_service.py
import logging
import os
...
# Import the Celery task we want to use
...
class FileUploadService:
    ...
    def __init__(self, db_session: AsyncSession):
        ...
        # Register all known entity schemas with the CSV processor
    ...
    async def handle_file_uploads_and_dispatch_tasks(
        self,
        ...
        """
        Validates files, creates database records, and dispatches Celery tasks for processing.
        This method replaces the old synchronous logic.
        """
        ...
        for file_path in file_paths:
            ...
            try:
                # 1. Detect entity type from file name or structure
                structure_validation = self.csv_processor.validate_csv_structure(
                    file_path, entity_type=None  # type: ignore
                )
                ...
                # 2. Create the file_uploads database record
                ...
                # 3. Dispatch the Celery task for background processing
                ...
                results["failed_files"].append(file_result)
                # Clean up temp file if dispatch fails
