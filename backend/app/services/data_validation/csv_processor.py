...
class CSVProcessor:
    ...
    def validate_csv_structure(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"CSV structure: {validation_result['row_count']} rows, {validation_result['column_count']} columns")
            ...
            if entity_type not in self.required_columns:
                validation_result['warnings'].append(f"No schema registered for entity type: {entity_type}")
                validation_result['is_valid'] = True
    ...
    def _find_mapped_column(self, required_col: str, entity_type: str, available_cols: List[str]) -> Optional[str]:
        """
    ...
    def process_csv_file(
        ...
        try:
            ...
            structure_validation = self.validate_csv_structure(file_path, entity_type, encoding)
            ...
            for chunk_df in chunk_iter:
                ...
                logger.info(f"Processed chunk: {len(chunk_df)} rows, total processed: {result['processed_rows']}")

            result['success'] = True
            logger.info(f"CSV processing complete: {result['processed_rows']}/{result['total_rows']} rows processed")
    ...
    def _process_chunk(
        ...
        for local_idx, (_index_label, row) in enumerate(mapped_df.iterrows()):
            ...
            try:
                ...
                if validate_data:
                    validation_result = self._validate_row_data(transformed_row, entity_type, row_number)
    ...
    def _apply_column_mappings(self, df: pd.DataFrame, entity_type: str) -> pd.DataFrame:
    ...
    def _transform_row_data(self, row_data: Dict[str, Any], entity_type: str) -> Dict[str, Any]:
