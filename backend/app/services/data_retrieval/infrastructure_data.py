...
class InfrastructureData:
    ...
    def __init__(self, session: AsyncSession):
        ...
        logger.debug(f"InfrastructureData service initialized with session: {session}")

    # Buildings
    async def get_all_buildings(self) -> List[Dict]:
        """Get all buildings with room counts"""
        logger.info("Retrieving all buildings with room counts")
        try:
            stmt = (
                select(Building)
            ...
            logger.debug(f"Retrieved {len(buildings)} buildings from database")
            ...
            building_data = [
                {
                    ...
                    "active_room_count": len(
                        [r for r in building.rooms if r.is_active]
                    ),
                    "total_capacity": sum(
            ...
            logger.info(f"Successfully processed {len(building_data)} buildings")
            return building_data

        except Exception as e:
            logger.error(f"Error retrieving all buildings: {str(e)}", exc_info=True)
            raise

    async def get_active_buildings(self) -> List[Dict]:
        """Get only active buildings"""
        logger.info("Retrieving active buildings only")
        try:
            stmt = (
                select(Building)
                .options(selectinload(Building.rooms))
            ...
            logger.debug(f"Retrieved {len(buildings)} active buildings from database")
            ...
            logger.info(f"Successfully processed {len(building_data)} active buildings")
            return building_data

        except Exception as e:
            logger.error(f"Error retrieving active buildings: {str(e)}", exc_info=True)
            raise

    async def get_building_by_id(self, building_id: UUID) -> Optional[Dict]:
        """Get building by ID with detailed room information"""
        logger.info(f"Retrieving building by ID: {building_id}")
        try:
            stmt = (
                select(Building)
            ...
            logger.info(
                f"Successfully retrieved building {building.name} ({building.code}) with {len(building.rooms)} rooms"
            )
            return building_data

        except Exception as e:
            logger.error(
                f"Error retrieving building by ID {building_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def get_all_room_types(self) -> List[Dict]:
        ...
        logger.info("Retrieving all room types with room counts")
        try:
            stmt = (
                select(RoomType)
            ...
            logger.debug(f"Retrieved {len(room_types)} room types from database")
            ...
            room_type_data = [
                {
                    ...
                    "active_room_count": len(
                        [r for r in room_type.rooms if r.is_active]
                    ),
                }
                for room_type in room_types
            ]

            logger.info(f"Successfully processed {len(room_type_data)} room types")
            return room_type_data

        except Exception as e:
            logger.error(f"Error retrieving all room types: {str(e)}", exc_info=True)
            raise

    async def get_active_room_types(self) -> List[Dict]:
        """Get only active room types"""
        logger.info("Retrieving active room types only")
        try:
            stmt = select(RoomType).where(RoomType.is_active).order_by(RoomType.name)
            result = await self.session.execute(stmt)
            room_types = result.scalars().all()

            logger.debug(f"Retrieved {len(room_types)} active room types from database")
            ...
            logger.info(
                f"Successfully processed {len(room_type_data)} active room types"
            )
            return room_type_data

        except Exception as e:
            logger.error(f"Error retrieving active room types: {str(e)}", exc_info=True)
            raise

    # Rooms
    async def get_all_rooms(self) -> List[Dict]:
        """Get all rooms with building and type information"""
        logger.info("Retrieving all rooms with building and type information")
        try:
            stmt = (
                select(Room)
            ...
            logger.debug(f"Retrieved {len(rooms)} rooms from database")
            ...
            room_data = [
                {
                    ...
                    "room_type_id": room.room_type_id if room.room_type_id else None,
            ...
            logger.info(f"Successfully processed {len(room_data)} rooms")
            return room_data

        except Exception as e:
            logger.error(f"Error retrieving all rooms: {str(e)}", exc_info=True)
            raise

    async def get_active_rooms(self) -> List[Dict]:
        """Get only active rooms"""
        logger.info("Retrieving active rooms only")
        try:
            stmt = (
                select(Room)
                .options(selectinload(Room.building), selectinload(Room.room_type))
            ...
            logger.debug(f"Retrieved {len(rooms)} active rooms from database")
            ...
            logger.info(f"Successfully processed {len(room_data)} active rooms")
            return room_data

        except Exception as e:
            logger.error(f"Error retrieving active rooms: {str(e)}", exc_info=True)
            raise

    async def get_rooms_by_building(self, building_id: UUID) -> List[Dict]:
        """Get rooms by building ID"""
        logger.info(f"Retrieving rooms for building ID: {building_id}")
        try:
            stmt = (
                select(Room)
            ...
            logger.debug(f"Retrieved {len(rooms)} rooms for building {building_id}")
            ...
            logger.info(
                f"Successfully processed {len(room_data)} rooms for building {building_id}"
            )
            return room_data

        except Exception as e:
            logger.error(
                f"Error retrieving rooms for building {building_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def get_rooms_by_type(self, room_type_id: UUID) -> List[Dict]:
        ...
        logger.info(f"Retrieving rooms for room type ID: {room_type_id}")
        try:
            stmt = (
                select(Room)
            ...
            logger.debug(f"Retrieved {len(rooms)} rooms for room type {room_type_id}")
            ...
            logger.info(
                f"Successfully processed {len(room_data)} rooms for room type {room_type_id}"
            )
            return room_data

        except Exception as e:
            logger.error(
                f"Error retrieving rooms for room type {room_type_id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def get_room_by_id(self, room_id: UUID) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Successfully retrieved room {room.code} ({room.name}) with {len(room.exam_rooms)} exam assignments and {len(room.allowed_exams)} allowed exams"
            )
            return room_data

        except Exception as e:
            logger.error(
                f"Error retrieving room by ID {room_id}: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_rooms_with_features(
        ...
        logger.info(
            f"Searching rooms with features: projector={has_projector}, ac={has_ac}, computers={has_computers}, "
            f"min_capacity={min_capacity}, building_id={building_id}, overbookable={overbookable}"
        )
        try:
            stmt = (
                select(Room)
            ...
            logger.debug(
                f"Retrieved {len(rooms)} rooms matching the specified features"
            )
            ...
            logger.info(
                f"Successfully processed {len(room_data)} rooms matching the specified features"
            )
            return room_data

        except Exception as e:
            logger.error(
                f"Error searching rooms with features: {str(e)}", exc_info=True
            )
            raise

    async def search_rooms(self, search_term: str) -> List[Dict]:
        """Search rooms by code or name"""
        logger.info(f"Searching rooms with term: '{search_term}'")
        try:
            ...
            logger.debug(
                f"Found {len(rooms)} rooms matching search term '{search_term}'"
            )
            ...
            logger.info(
                f"Successfully processed {len(room_data)} rooms matching search term '{search_term}'"
            )
            return room_data

        except Exception as e:
            logger.error(
                f"Error searching rooms with term '{search_term}': {str(e)}",
                exc_info=True,
            )
            raise
    ...
    async def get_exam_room_assignments(
        ...
        logger.info(
            f"Retrieving exam room assignments: exam_id={exam_id}, room_id={room_id}"
        )
        try:
            ...
            logger.debug(f"Retrieved {len(assignments)} exam room assignments")
            ...
            logger.info(
                f"Successfully processed {len(assignment_data)} exam room assignments"
            )
            return assignment_data

        except Exception as e:
            logger.error(
                f"Error retrieving exam room assignments: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_exam_allowed_rooms(
        ...
        logger.info(
            f"Retrieving exam allowed rooms: exam_id={exam_id}, room_id={room_id}"
        )
        try:
            ...
            logger.debug(f"Retrieved {len(assignments)} exam allowed room assignments")
            ...
            logger.info(
                f"Successfully processed {len(assignment_data)} exam allowed room assignments"
            )
            return assignment_data

        except Exception as e:
            logger.error(
                f"Error retrieving exam allowed rooms: {str(e)}", exc_info=True
            )
            raise
    ...
    async def get_building_statistics(self) -> List[Dict]:
        ...
        logger.info("Retrieving building statistics")
        try:
            stmt = (
                select(
                    Building.id,
                    ...
                    func.count(func.nullif(Room.is_active, False)).label(
                        "active_rooms"
                    ),
            ...
            logger.debug(f"Retrieved statistics for {len(stats)} buildings")
            ...
            logger.info(
                f"Successfully processed statistics for {len(stats_data)} buildings"
            )
            return stats_data

        except Exception as e:
            logger.error(
                f"Error retrieving building statistics: {str(e)}", exc_info=True
            )
            raise

    async def get_room_type_statistics(self) -> List[Dict]:
        """Get room type statistics"""
        logger.info("Retrieving room type statistics")
        try:
            stmt = (
                select(
                    RoomType.id,
            ...
            logger.debug(f"Retrieved statistics for {len(stats)} room types")

            stats_data = [
                {
                    "room_type_id": stat.id,
            ...
            logger.info(
                f"Successfully processed statistics for {len(stats_data)} room types"
            )
            return stats_data

        except Exception as e:
            logger.error(
                f"Error retrieving room type statistics: {str(e)}", exc_info=True
            )
            raise

    async def get_room_utilization_summary(self) -> Dict:
        """Get room utilization summary"""
        logger.info("Retrieving room utilization summary")
        try:
            total_stmt = select(
                func.count(Room.id).label("total_rooms"),
            ...
            features_stmt = select(
                func.count(
                    func.nullif(Room.has_projector & Room.is_active, False)
                ).label("with_projector"),
                func.count(func.nullif(Room.has_ac & Room.is_active, False)).label(
                    "with_ac"
                ),
                func.count(
                    func.nullif(Room.has_computers & Room.is_active, False)
                ).label("with_computers"),
                func.count(
                    func.nullif(Room.overbookable & Room.is_active, False)
                ).label("overbookable"),
            )
            ...
            logger.info(
                f"Room utilization summary: {total_rooms} total rooms, {active_rooms} active, "
                f"total capacity: {total_capacity}, exam capacity: {total_exam_capacity}"
            )
            return summary_data

        except Exception as e:
            logger.error(
                f"Error retrieving room utilization summary: {str(e)}", exc_info=True
            )
            raise
